 ...
 */
namespace voduploadsdk\uploader;
use voduploadsdk\vod\Request\V20170321 as vod;
use OSS\OssClient;
use OSS\Core\OssUtil;
use voduploadsdk\core\Http\HttpHelper;
use voduploadsdk\core\Profile\DefaultProfile;
use voduploadsdk\core\DefaultAcsClient;
...
class AliyunVodUploader
{
    ...
    public function __construct($accessKeyId, $accessKeySecret, $apiRegionId=null)
    {
        ...
        if (empty($apiRegionId)) {
            ...
            $this->apiRegionId = config('OSS.OSS_APIREGION_ID');
        }
        else {
    ...
    public function uploadLocalVideo($uploadVideoRequest)
    {
        ...
        $playInfo = $this->getPlayInfo($uploadInfo->VideoId);
        ...
        $arr = [
            'playUrl' => $playInfo['playInfo']->PlayInfoList->PlayInfo[0]->PlayURL ,
            'playAuth' => $playInfo['playAuth']->PlayAuth ,
            'videoId' => $playInfo['playInfo']->VideoBase->VideoId
        ];
        return $arr;
    }

    public function getVideoInfo($videoId){
        $videoInfo = $this->getMezzanineInfo($videoId);
        return $videoInfo;
    }
    ...
    private function getPlayInfo($videoId)
    {
        $request = new vod\GetPlayInfoRequest();
        $request->setVideoId($videoId);
        $request->setAuthTimeout(config("vod_expires_time"));
        $request->setAcceptFormat('JSON');
        $playInfo = $this->vodClient->getAcsResponse($request);
        $playAuth = $this->getPlayAuth($videoId);
        $data = [
            'playInfo' => $playInfo,
            'playAuth' => $playAuth,
        ];
    ...
    private function getPlayAuth($videoId){
    ...
    private function getMezzanineInfo($videoId){
